...
def snapshot() -> dict[str, Any]:
    ...
    try:
        ...
        rpm = [
            {"fan": f"{n}:{e.label or i}", "rpm": e.current}
            for n, es in fans.items()
    ...
    if not found:  # fall back to globbing, some boards do not report properly
        for pat in ("/dev/ttyUSB*", "/dev/ttyACM*"):
            for dev in sorted(glob.glob(pat)):
                found.append(
                    {"port": dev, "description": "(detected by path)", "hwid": ""}
                )
...
async def serial_io(
    port: str, baud: int = 115200, seconds: float = 3.0, send: str | None = None
) -> dict:
    ...
    if not os.path.exists(port) and not port.upper().startswith("COM"):
        return {
            "ok": False,
            "error": f"{port} does not exist. Is the board plugged in?",
        }
    ...
    try:
        ...
        baud = 115200
    logger.info(
        f"[Serial] {port} @ {baud} for {seconds:.1f}s{' (sending)' if send else ''}"
    )
    try:
